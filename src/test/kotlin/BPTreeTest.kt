...
class BPTreeTest {
    ...
    @Test
    fun `2When inserting elements beyond degree should keep all elements and still remain valid`() {
        val tree = BPTree(degree = 3)
...
//            println("---")
        ...
        val expectedValues = keys.map { "$it" }
        assertThat(values).containsExactlyElementsOf(expectedValues)
        keys.forEach { assertThat(tree.get(it)).isEqualTo("$it") }
        tree.validate()
    }
